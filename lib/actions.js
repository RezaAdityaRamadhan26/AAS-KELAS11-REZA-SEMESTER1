...
export async function getUserById(id) {
  const [rows] = await withTimeout(
    db.execute("SELECT * FROM users WHERE id = ? LIMIT 1", [id])
  );
  return rows.length ? rows[0] : null;
}
...
export async function createUser({
  ...
  try {
    const [result] = await db.execute(
      "INSERT INTO users (username, password, full_name, role, class_grade, email, phone, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [username, passwordHash, full_name, role, class_grade, email, phone, 1]
    );
...
export async function approveLoan(loanId, adminId) {
  try {
    // Update loan status to 'dipinjam' dengan approved_by dan approved_at
    await db.execute(
      "UPDATE loans SET status = 'dipinjam', approved_by = ?, approved_at = NOW() WHERE id = ?",
      [Number(adminId), Number(loanId)]
    );

    // Get loan details and decrease book stock
